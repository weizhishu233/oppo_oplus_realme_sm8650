...
cd kernel_workspace
git clone --depth=1 https://github.com//android_kernel_common_oneplus_sm8650 -b oneplus/sm8650_b_16.0.0_oneplus12 common
echo ">>> 初始化仓库完成"
...
if [[ $KSU_BRANCH == [yYrR] ]]; then
  ...
  echo 'CONFIG_KSU_FULL_NAME_FORMAT="%TAG_NAME%-%COMMIT_SHA%@zhishu"' >> ./common/arch/arm64/configs/gki_defconfig
elif [[ "$KSU_BRANCH" == "n" || "$KSU_BRANCH" == "N" ]]; then
